...
test.describe('Forms', () => {
  ...
  test.describe('Registration Form', () => {
    test('should display registration form', async ({ page }) => {
      ...
      const nameField = page.locator('input[name="name"], input[name="nome"], input[type="text"]').first();
      ...
      for (const pagePath of pages) {
        ...
        for (let i = 0; i < inputCount; i++) {
          ...
          const hasLabel = await input.evaluate((el) => {
            ...
            return hasAssociatedLabel || hasAriaLabel || hasAriaLabelledBy || hasPlaceholder;
          });
  ...
  test.describe('Form Validation', () => {
    test('should validate email format', async ({ page }) => {
      ...
      if (await emailInput.count() > 0) {
        ...
        const validityAfter = await emailInput.evaluate((el: HTMLInputElement) => el.validity.valid);
