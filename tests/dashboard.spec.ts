...
test.describe("Dashboard", () => {
	test("should handle authentication requirement", async ({ page }) => {
		...
		expect(
			hasAuthForm || hasDashboardContent || currentUrl.includes("/"),
		).toBeTruthy();
	});
	...
	test("should display dashboard components when authenticated", async ({
		...
		await expect(
			page.locator("text=Bem-vindo ao seu painel de controle financeiro"),
		).toBeVisible();

		// Check for dashboard components (these may load asynchronously)
		const summarySection = page.locator(
			'[data-testid="dashboard-summary"], .dashboard-summary',
		);
		...
		const chartSections = page.locator(
			"text=Receitas vs Despesas, text=Despesas por Categoria",
		);
		...
		const transactionsSection = page.locator(
			"text=Transações Recentes, text=Recent, text=Recentes",
		);
