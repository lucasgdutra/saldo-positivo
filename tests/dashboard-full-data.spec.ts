...
test.describe("Dashboard with Full Historical Data", () => {
	test("should display correct financial summary with 20 months of data", async ({
		page,
	}) => {
	...
	test("should display charts and visualizations with historical data", async ({
		page,
	}) => {
		const helpers = createTestHelpers(page);
		const seedUser = helpers.getSeedUser();
		...
		const chartContainers = page.locator(
			'canvas, svg[class*="recharts"], .recharts-wrapper, [data-testid*="chart"]',
		);
		...
		const chartSections = page.locator(
			"text=Receitas vs Despesas, text=Despesas por Categoria, text=Histórico",
		);
		...
		const transactionsSection = page.locator(
			"text=Transações Recentes, text=Recent, text=Histórico de Transações",
		);
