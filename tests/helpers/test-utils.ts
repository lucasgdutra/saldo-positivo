...
export class TestHelpers {
  ...
  async clickButton(text: string) {
    const button = this.page.locator(`button:has-text("${text}"), input[type="submit"][value="${text}"], a[role="button"]:has-text("${text}")`).first();
    ...
    for (let i = 0; i < inputCount; i++) {
      ...
      if (await input.isVisible()) {
        const hasAccessibleName = await input.evaluate((el) => {
          ...
          return !!(hasLabel || hasAriaLabel || hasAriaLabelledBy || hasPlaceholder || hasName || parentLabel);
        });
        
        // Only fail if it's a visible form field without any accessible name
        if (!hasAccessibleName) {
          console.warn(`Input without accessible name found: ${await input.getAttribute('name') || await input.getAttribute('type') || 'unknown'}`);
        }
