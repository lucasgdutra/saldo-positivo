...
test.describe('Real Authentication Tests', () => {
  test('should login with seed user and see dashboard data', async ({ page }) => {
    ...
    await page.fill('input[type="password"], input[name="password"]', seedUser.password);
  ...
  test('should show seed data after successful login', async ({ page }) => {
    ...
    const hasNoDataMessage = await page.locator('text=Sem dados financeiros').count() > 0;
