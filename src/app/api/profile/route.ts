...
export async function PUT(request: NextRequest) {
  try {
    ...
    const updatedProfile = await profileService.updateProfile(session.user.id, updateData);
    ...
    if (error instanceof Error && error.message === 'Este email já está sendo usado por outro usuário') {
      return NextResponse.json(
        { error: error.message },
        { status: 400 }
      );
