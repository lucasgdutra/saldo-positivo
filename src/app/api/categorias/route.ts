...
export async function GET() {
	try {
		...
		const categorias = await categoryService.getCategoriesByUser(
			session.user.id,
		);
	...
	try {
		...
		if (
			...
			return new NextResponse(
				JSON.stringify({
					error:
						"Não é possível excluir a categoria, pois ela está associada a despesas existentes.",
				}),
				{ status: 409 }, // Conflict
			);
		}
