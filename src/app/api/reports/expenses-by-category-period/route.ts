...
export async function GET(request: NextRequest) {
	try {
		...
		const { category, expenses } =
			await expenseService.getExpensesByCategoryAndPeriodReport(
				userId,
				categoryId,
				start,
				end,
		...
		const totalAmount = expenses.reduce((acc: Decimal, expense) => {
		...
		if (
			...
			return new NextResponse(
				JSON.stringify({
					error: "Categoria não encontrada ou acesso não permitido.",
				}),
				{
