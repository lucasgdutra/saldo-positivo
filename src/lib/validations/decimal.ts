...
export const DecimalValidation = {
	...
	number: (fieldName = "valor") =>
		z
			.number()
			.min(
				DecimalValidation.MIN_VALUE,
				`${fieldName} deve ser maior ou igual a R$ 0,00`,
			)
			.max(
				DecimalValidation.MAX_VALUE,
				`${fieldName} não pode ser maior que R$ 99.999.999,99`,
			)
	...
	flexible: (fieldName = "valor") => {
		...
		return z.union([stringSchema, numberSchema]).refine(
			(val) => {
	...
	_validateDecimal: (value: number): boolean => {
		...
		if (
			value < DecimalValidation.MIN_VALUE ||
			value > DecimalValidation.MAX_VALUE
		) {
			return false;
		}
