...
const VALOR_SALARIO_MINIMO = 1518.0;
		...
		<form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
			...
			<div>
				...
				>
					...
					<option value="ate-1-salario">
						Até 1 salário mínimo (R${" "}
						{VALOR_SALARIO_MINIMO.toLocaleString("pt-BR", {
							minimumFractionDigits: 2,
						})}
						)
					</option>
					<option value="1-a-2-salarios">
						1 a 2 salários mínimos (R${" "}
						{VALOR_SALARIO_MINIMO.toLocaleString("pt-BR", {
							minimumFractionDigits: 2,
						})}{" "}
						- R${" "}
						{(VALOR_SALARIO_MINIMO * 2).toLocaleString("pt-BR", {
							minimumFractionDigits: 2,
						})}
						)
					</option>
					<option value="2-a-3-salarios">
						2 a 3 salários mínimos (R${" "}
						{(VALOR_SALARIO_MINIMO * 2).toLocaleString("pt-BR", {
							minimumFractionDigits: 2,
						})}{" "}
						- R${" "}
						{(VALOR_SALARIO_MINIMO * 3).toLocaleString("pt-BR", {
							minimumFractionDigits: 2,
						})}
						)
					</option>
					<option value="3-a-5-salarios">
						3 a 5 salários mínimos (R${" "}
						{(VALOR_SALARIO_MINIMO * 3).toLocaleString("pt-BR", {
							minimumFractionDigits: 2,
						})}{" "}
						- R${" "}
						{(VALOR_SALARIO_MINIMO * 5).toLocaleString("pt-BR", {
							minimumFractionDigits: 2,
						})}
						)
					</option>
					<option value="5-a-10-salarios">
						5 a 10 salários mínimos (R${" "}
						{(VALOR_SALARIO_MINIMO * 5).toLocaleString("pt-BR", {
							minimumFractionDigits: 2,
						})}{" "}
						- R${" "}
						{(VALOR_SALARIO_MINIMO * 10).toLocaleString("pt-BR", {
							minimumFractionDigits: 2,
						})}
						)
					</option>
					<option value="acima-10-salarios">
						Acima de 10 salários mínimos (R${" "}
						{(VALOR_SALARIO_MINIMO * 10).toLocaleString("pt-BR", {
							minimumFractionDigits: 2,
						})}
						+)
			...
			<div>
				...
				{errors.usageMotivation && (
					<p className="mt-1 text-sm text-red-600">
						{errors.usageMotivation.message}
					</p>
				)}
			</div>
			...
			{watchUsageMotivation === "outro" && (
				<div>
					...
					{errors.customMotivation && (
						<p className="mt-1 text-sm text-red-600">
							{errors.customMotivation.message}
						</p>
					)}
				</div>
			)}
