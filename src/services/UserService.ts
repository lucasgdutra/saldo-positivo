...
import {
	...
	Revenue,
	User,
} from "@prisma/client"; // Adiciona Revenue, Expense
...
class UserService {
	 ...
	 */
	async getUserById(userId: string): Promise<Omit<User, "balance"> | null> {
		// Ajusta o tipo de retorno
	...
	async getUserByIdWithBalance(
		...
		if (!user) {
			console.warn(
				`UserService: Usuário com ID ${userId} não encontrado ao buscar com saldo.`,
			);
		}
		return user;
	}
	...
	async updateUser(
		userId: string,
		data: Partial<Omit<User, "id" | "balance" | "createdAt" | "updatedAt">>,
	): Promise<User> {
		console.log(`UserService: Atualizando usuário com ID: ${userId}`);
		const user = await this.getUserById(userId);
		if (!user) {
			console.error(
				`UserService: Tentativa de atualizar usuário não existente: ${userId}`,
			);
		...
		if (Object.keys(updateData).length === 0) {
			console.warn(
				`UserService: Nenhuma informação válida para atualizar para o usuário ${userId}.`,
			);
			return user; // Retorna o usuário sem alterações se nada for fornecido
		}

		console.log(
			`UserService: Dados para atualização para usuário ${userId}:`,
			updateData,
		);
	...
	async getUserBalance(userId: string): Promise<Decimal> {
		...
		if (!balance) {
			...
			console.error(
				`UserService: Saldo não encontrado para o usuário ID: ${userId}. Retornando 0.`,
			);
			// throw new Error('Saldo do usuário não encontrado');
			return new Decimal(0); // Ou lançar erro dependendo da regra
		}
		console.log(
			`UserService: Saldo encontrado para ${userId}: ${balance.totalAmount}`,
		);
	...
	async updateUserBalance(
		...
		console.log(
			`UserService: Recalculando saldo para usuário ID: ${userId}, Dentro da transação: ${!!tx}`,
		);
		...
		const updateLogic = async (
			...
			const { RevenueRepository } = await import(
				"../repositories/RevenueRepository"
			);
			...
			const totalRevenue = allRevenues.reduce(
				(sum: Decimal, revenue: Revenue) =>
					sum.add(new Decimal(revenue.amount)),
				new Decimal(0),
			);
			const totalExpense = allExpenses.reduce(
				(sum: Decimal, expense: Expense) =>
			...
			console.log(
				`UserService (updateLogic): Totais recalculados para ${userId}: Receitas=${totalRevenue}, Despesas=${totalExpense}, Saldo=${newCalculatedBalance}`,
			);
			...
			if (!currentBalance) {
				console.warn(
					`UserService (updateLogic): Objeto Balance não encontrado para ${userId}. Criando um novo.`,
				);
				...
				console.log(
					`UserService (updateLogic): Novo objeto Balance criado para ${userId}.`,
				);
			} else {
				...
				console.log(
					`UserService (updateLogic): Balance atualizado para ${userId}.`,
				);
			}
			...
			if (!finalUser) {
				console.error(
					`UserService (updateLogic): Usuário ${userId} não encontrado após atualização de saldo!`,
				);
				throw new Error("Falha ao buscar usuário após atualização de saldo.");
			}
			const { balance, ...userWithoutBalance } = finalUser;
			console.log(
				`UserService (updateLogic): Retornando usuário ${userId} após atualização de saldo.`,
			);
	...
	async setUserBalance(userId: string, newBalance: Decimal): Promise<User> {
		console.log(
			`UserService: Definindo saldo diretamente para usuário ID: ${userId}, Novo Saldo: ${newBalance}`,
		);
		if (newBalance.isNaN() || !newBalance.isFinite()) {
			console.error(
				`UserService: Tentativa de definir saldo inválido (${newBalance}) para usuário ${userId}.`,
			);
		...
		if (!userExists) {
			console.error(
				`UserService: Tentativa de definir saldo para usuário não existente: ${userId}`,
			);
			throw new Error("Usuário não encontrado");
		}
		...
		if (newBalance.isNegative()) {
			console.warn(
				`UserService: Definindo um saldo negativo (${newBalance}) para o usuário ${userId}. Verifique as regras de negócio.`,
			);
		}
		...
		await this.userRepository.updateBalancePartial(userId, {
			totalAmount: newBalance.toNumber(),
		});
		console.log(
			`UserService: Saldo definido diretamente com sucesso para ${userId}.`,
		);
		...
		if (!updatedUser) {
			...
			console.error(
				`UserService: Falha ao buscar usuário ${userId} após definir saldo.`,
			);
	...
	async recalculateBalance(userId: string): Promise<Balance> {
		console.log(
			`UserService: Iniciando recálculo de saldo para o usuário ID: ${userId}`,
		);
		...
		return this.prisma.$transaction(async (tx) => {
			...
			if (!user) {
				console.error(
					`UserService (recalculate): Usuário ${userId} não encontrado para recálculo.`,
				);
			...
			const newCalculatedBalance = totalRevenue.sub(totalExpense);
			console.log(
				`UserService (recalculate): Saldo recalculado para ${userId}: Receitas=${totalRevenue}, Despesas=${totalExpense}, Saldo=${newCalculatedBalance}`,
			);

			// 5. Buscar o objeto Balance atual
			let currentBalance = await userRepo.getBalance(userId);
			const previousBalanceAmount = currentBalance?.totalAmount
				? new Decimal(currentBalance.totalAmount)
				: new Decimal(0); // Guarda o saldo anterior

			// 6. Atualizar ou criar o objeto Balance
			if (!currentBalance) {
				console.warn(
					`UserService (recalculate): Objeto Balance não encontrado para ${userId}. Criando um novo.`,
				);
				...
				console.log(
					`UserService (recalculate): Novo objeto Balance criado para ${userId} com saldo ${newCalculatedBalance}, receitas ${totalRevenue}, despesas ${totalExpense}.`,
				);
			} else {
				// Atualiza apenas se o valor calculado for diferente do atual ou outros totais mudaram
				const currentBalanceAmountDecimal = new Decimal(
					currentBalance.totalAmount,
				);
				const currentTotalRevenuesDecimal = new Decimal(
					currentBalance.totalRevenues,
				);
				const currentTotalExpensesDecimal = new Decimal(
					currentBalance.totalExpenses,
				);

				if (
					!currentBalanceAmountDecimal.equals(newCalculatedBalance) ||
					...
					console.log(
						`UserService (recalculate): Saldo/Totais atualizados no objeto Balance para ${userId}. Novo saldo: ${newCalculatedBalance}, Receitas: ${totalRevenue}, Despesas: ${totalExpense}`,
					);
				} else {
					console.log(
						`UserService (recalculate): Saldo e totais recalculados são iguais aos atuais. Nenhuma atualização necessária no Balance.`,
					);
				}
			}
			...
			if (!finalBalance) {
				...
				console.error(
					`UserService (recalculate): Falha crítica ao buscar balance final para ${userId} após recálculo.`,
				);
				throw new Error("Falha ao obter o saldo final após o recálculo.");
			}

			console.log(
				`UserService: Recálculo de saldo concluído para ${userId}. Saldo final: ${finalBalance.totalAmount}`,
			);
			return finalBalance;
		});
	}
	...
	async getBalanceHistory(
		userId: string,
	): Promise<{ month: string; receitas: number; despesas: number }[]> {
		console.log(
			`UserService: Buscando histórico de saldo para o usuário ID: ${userId}`,
		);
		try {
			...
			const dadosMensais = await Promise.all(
				meses.map(async (mes) => {
					const [despesasResult, receitasResult] =
					...
					const receitas =
						receitasResult._sum.amount instanceof Decimal
							? receitasResult._sum.amount.toNumber()
							: (receitasResult._sum.amount ?? 0);

					const despesas =
						despesasResult._sum.amount instanceof Decimal
			...
			console.error(
				`UserService: Erro ao buscar histórico de saldo para ${userId}:`,
				error,
			);
