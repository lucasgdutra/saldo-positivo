...
import { type CreateUserApiData, CreateUserApiSchema } from "@/lib/validations";
...
export class AuthService {
	...
	async register(data: CreateUserApiData): Promise<Omit<User, "password">> {
		...
		try {
			...
			console.error(
				`Erro ao criar categorias padrão para o usuário ${user.id}:`,
				error,
			);
		...
		try {
			...
			console.error(
				`Erro ao criar balance inicial para o usuário ${user.id}:`,
				error,
			);
		}
