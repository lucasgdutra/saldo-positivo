...
import { Prisma, PrismaClient, Revenue } from "@prisma/client";
...
class RevenueService {
	...
	async createRevenue(data: {
		...
		if (
			revenueAmount.isNaN() ||
			!revenueAmount.isFinite() ||
			revenueAmount.isNegative() ||
			revenueAmount.isZero()
		) {
			console.error(
				`RevenueService: Tentativa de criar receita com valor inválido (${amount}) para usuário ${userId}.`,
			);
			throw new Error("O valor da receita deve ser um número positivo.");
		}
		if (!date || !(date instanceof Date) || isNaN(date.getTime())) {
			console.error(
				`RevenueService: Data inválida fornecida para criação de receita: ${date}`,
			);
			throw new Error("Data inválida fornecida.");
		}

		console.log(
			`RevenueService: Iniciando criação de receita para usuário ${userId}, valor: ${revenueAmount}`,
		);
		...
		return this.prisma.$transaction(async (tx: TransactionClient) => {
			...
			console.log(
				`RevenueService (TX): Criando registro de receita...`,
				createData,
			);
			...
			console.log(
				`RevenueService (TX): Recalculando saldo do usuário ${userId}.`,
			);
			await this.userService.updateUserBalance(userId, tx); // Passa tx aqui
			console.log(
				`RevenueService (TX): Saldo do usuário ${userId} recalculado.`,
			);
	...
	async getRevenueById(id: string, userId: string): Promise<Revenue | null> {
		console.log(
			`RevenueService: Buscando receita ID ${id} para usuário ${userId}.`,
		);
		const revenue = await this.revenueRepository.findById(id);

		if (revenue && revenue.userId !== userId) {
			console.error(
				`RevenueService: Receita ${id} encontrada, mas pertence a outro usuário (requisitado por ${userId}).`,
			);
	 ...
	 */
	async getRevenuesByUserAndPeriod(
		userId: string,
		startDate: Date,
		endDate: Date,
	): Promise<Revenue[]> {
		console.log(
			`RevenueService: Listando receitas para usuário ${userId} entre ${startDate.toISOString()} e ${endDate.toISOString()}.`,
		);
		return this.revenueRepository.findByUserIdAndPeriod(
			userId,
			startDate,
			endDate,
		);
	...
	async updateRevenue(
		id: string,
		userId: string,
		data: { amount?: number | Decimal; description?: string; date?: Date },
	): Promise<Revenue> {
		...
		if (amount !== undefined) {
			...
			if (
				...
				console.error(
					`RevenueService: Tentativa de atualizar receita ${id} com valor inválido (${amount}) para usuário ${userId}.`,
				);
				throw new Error("O valor da receita deve ser um número positivo.");
			}
		}
		if (
			...
			console.error(
				`RevenueService: Data inválida fornecida para atualização da receita ${id}: ${date}`,
			);
			throw new Error("Data inválida fornecida.");
		}

		console.log(
			`RevenueService: Iniciando atualização da receita ID ${id} para usuário ${userId}.`,
		);
		...
		return this.prisma.$transaction(async (tx: TransactionClient) => {
			...
			if (!originalRevenue) {
				console.error(
					`RevenueService (TX): Receita ${id} não encontrada para atualização.`,
				);
				throw new Error("Receita não encontrada.");
			}
			if (originalRevenue.userId !== userId) {
				console.error(
					`RevenueService (TX): Usuário ${userId} tentando atualizar receita ${id} de outro usuário.`,
				);
				throw new Error("Acesso não permitido para atualizar esta receita.");
			}
			const oldAmount = originalRevenue.amount;
			console.log(
				`RevenueService (TX): Receita ${id} encontrada. Valor antigo: ${oldAmount}.`,
			);
			...
			if (Object.keys(updateData).length === 0) {
				console.warn(
					`RevenueService (TX): Nenhuma alteração detectada para a receita ${id}. Retornando original.`,
				);
				return originalRevenue; // Nenhuma alteração necessária
			}

			console.log(
				`RevenueService (TX): Atualizando receita ${id} com dados:`,
				updateData,
			);
			const updatedRevenue = await revenueRepoTx.update(id, updateData);
			console.log(
				`RevenueService (TX): Receita ${id} atualizada. Novo valor: ${updatedRevenue.amount}.`,
			);
			...
			if (newAmount !== undefined && !newAmount.equals(oldAmount)) {
				console.log(
					`RevenueService (TX): Valor da receita alterado. Recalculando saldo do usuário ${userId}.`,
				);
				...
				console.log(
					`RevenueService (TX): Valor da receita ${id} não alterado. Saldo do usuário ${userId} não precisa de ajuste.`,
				);
			}
	...
	async deleteRevenue(id: string, userId: string): Promise<Revenue> {
		console.log(
			`RevenueService: Iniciando deleção da receita ID ${id} para usuário ${userId}.`,
		);
		...
		return this.prisma.$transaction(async (tx: TransactionClient) => {
			...
			if (!revenueToDelete) {
				console.error(
					`RevenueService (TX): Receita ${id} não encontrada para deleção.`,
				);
				throw new Error("Receita não encontrada.");
			}
			if (revenueToDelete.userId !== userId) {
				console.error(
					`RevenueService (TX): Usuário ${userId} tentando deletar receita ${id} de outro usuário.`,
				);
				throw new Error("Acesso não permitido para deletar esta receita.");
			}
			const amountToDelete = revenueToDelete.amount;
			console.log(
				`RevenueService (TX): Receita ${id} encontrada. Valor: ${amountToDelete}.`,
			);
			...
			console.log(
				`RevenueService (TX): Recalculando saldo do usuário ${userId} após deleção.`,
			);
			await this.userService.updateUserBalance(userId, tx);
			console.log(
				`RevenueService (TX): Saldo do usuário ${userId} recalculado.`,
	 ...
	 */
	async getRecentRevenues(
		userId: string,
		limit: number = 5,
	): Promise<Revenue[]> {
		console.log(
			`RevenueService: Buscando ${limit} receitas recentes para usuário ${userId}.`,
		);
		try {
			// Utiliza o método do repositório que já busca as recentes com limite
			const recentRevenues = await this.revenueRepository.findRecentByUserId(
				userId,
				limit,
			);
			console.log(
				`RevenueService: ${recentRevenues.length} receitas recentes encontradas para ${userId}.`,
			);
			return recentRevenues;
		} catch (error) {
			console.error(
				`RevenueService: Erro ao buscar receitas recentes para ${userId}:`,
				error,
			);
