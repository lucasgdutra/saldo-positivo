...
type ExtendedTransactionClient = Parameters<
	Parameters<PrismaClientWithExtensions["$transaction"]>[0]
>[0];
...
class ExpenseService {
	...
	async createExpense(data: {
		...
		if (
			expenseAmount.isNaN() ||
			!expenseAmount.isFinite() ||
			expenseAmount.isNegative() ||
			expenseAmount.isZero()
		) {
			console.error(
				`ExpenseService: Tentativa de criar despesa com valor inválido (${amount}) para usuário ${userId}.`,
			);
			throw new Error("O valor da despesa deve ser um número positivo.");
		}
		if (!date || !(date instanceof Date) || isNaN(date.getTime())) {
			console.error(
				`ExpenseService: Data inválida fornecida para criação de despesa: ${date}`,
			);
			throw new Error("Data inválida fornecida.");
		}
		if (!categoryId) {
			console.error(
				`ExpenseService: ID da categoria não fornecido para criação de despesa para usuário ${userId}.`,
			);
		...
		if (!category) {
			console.error(
				`ExpenseService: Categoria ID ${categoryId} não encontrada ou não pertence ao usuário ${userId}.`,
			);
			throw new Error("Categoria inválida ou não encontrada.");
		}

		console.log(
			`ExpenseService: Iniciando criação de despesa para usuário ${userId}, valor: ${expenseAmount}, categoria: ${categoryId}`,
		);
		...
		return this.prisma.$transaction(async (tx: ExtendedTransactionClient) => {
			...
			console.log(
				`ExpenseService (TX): Criando registro de despesa...`,
				createData,
			);
			...
			console.log(
				`ExpenseService (TX): Atualizando saldo do usuário ${userId}.`,
			);
			...
			console.log(
				`ExpenseService (TX): Saldo do usuário ${userId} atualizado.`,
			);
			...
			const createdExpenseWithCategory = await expenseRepoTx.findById(
				newExpense.id,
			);
			if (!createdExpenseWithCategory) {
				// Isso não deveria acontecer
				console.error(
					`ExpenseService (TX): Falha ao re-buscar despesa ${newExpense.id} após criação.`,
				);
	...
	async getExpenseById(
		...
		console.log(
			`ExpenseService: Buscando despesa ID ${id} para usuário ${userId}.`,
		);
		...
		if (expense.userId !== userId) {
			console.error(
				`ExpenseService: Despesa ${id} encontrada, mas pertence a outro usuário (requisitado por ${userId}).`,
			);
	 ...
	 */
	async getExpensesByUserAndPeriod(
		userId: string,
		startDate: Date,
		endDate: Date,
	): Promise<Prisma.ExpenseGetPayload<{ include: { category: true } }>[]> {
		console.log(
			`ExpenseService: Listando despesas para usuário ${userId} entre ${startDate.toISOString()} e ${endDate.toISOString()}.`,
		);
		// findByUserIdAndPeriod já inclui categoria
		return this.expenseRepository.findByUserIdAndPeriod(
			userId,
			startDate,
			endDate,
		);
	...
	async updateExpense(
		...
		if (amount !== undefined) {
			...
			if (
				...
				console.error(
					`ExpenseService: Tentativa de atualizar despesa ${id} com valor inválido (${amount}) para usuário ${userId}.`,
				);
				throw new Error("O valor da despesa deve ser um número positivo.");
			}
		}
		if (
			...
			console.error(
				`ExpenseService: Data inválida fornecida para atualização da despesa ${id}: ${date}`,
			);
			throw new Error("Data inválida fornecida.");
		}
		...
		if (categoryId) {
			...
			if (!category) {
				console.error(
					`ExpenseService: Nova categoria ID ${categoryId} não encontrada ou não pertence ao usuário ${userId} para atualização da despesa ${id}.`,
				);
		...
		console.log(
			`ExpenseService: Iniciando atualização da despesa ID ${id} para usuário ${userId}.`,
		);
		...
		return this.prisma.$transaction(async (tx: ExtendedTransactionClient) => {
			...
			if (!originalExpense) {
				console.error(
					`ExpenseService (TX): Despesa ${id} não encontrada para atualização.`,
				);
				throw new Error("Despesa não encontrada.");
			}
			if (originalExpense.userId !== userId) {
				console.error(
					`ExpenseService (TX): Usuário ${userId} tentando atualizar despesa ${id} de outro usuário.`,
				);
				throw new Error("Acesso não permitido para atualizar esta despesa.");
			}
			const oldAmount = originalExpense.amount;
			console.log(
				`ExpenseService (TX): Despesa ${id} encontrada. Valor antigo: ${oldAmount}.`,
			);
			...
			if (Object.keys(updateData).length === 0) {
				console.warn(
					`ExpenseService (TX): Nenhuma alteração detectada para a despesa ${id}. Retornando original.`,
				);
				...
				if (!currentExpenseWithCategory) {
					// A categoria pode ser null se a relação for opcional
			...
			console.log(
				`ExpenseService (TX): Atualizando despesa ${id} com dados:`,
				updateData,
			);
			const updatedExpense = await expenseRepoTx.update(id, updateData);
			console.log(
				`ExpenseService (TX): Despesa ${id} atualizada. Novo valor: ${updatedExpense.amount}.`,
			);
			...
			if (newAmount !== undefined && !newAmount.equals(oldAmount)) {
				console.log(
					`ExpenseService (TX): Valor da despesa alterado. Recalculando saldo do usuário ${userId}.`,
				);
				await this.userService.updateUserBalance(userId, tx);
				console.log(
					`ExpenseService (TX): Saldo do usuário ${userId} recalculado.`,
				);
			} else {
				console.log(
					`ExpenseService (TX): Valor da despesa ${id} não alterado. Saldo do usuário ${userId} não precisa de ajuste.`,
				);
			}

			// Retorna a despesa atualizada com a categoria
			const finalExpenseWithCategory = await expenseRepoTx.findById(
				updatedExpense.id,
			);
			if (!finalExpenseWithCategory) {
				console.error(
					`ExpenseService (TX): Falha ao re-buscar despesa ${updatedExpense.id} após atualização.`,
				);
	...
	async deleteExpense(id: string, userId: string): Promise<Expense> {
		console.log(
			`ExpenseService: Iniciando deleção da despesa ID ${id} para usuário ${userId}.`,
		);
		...
		return this.prisma.$transaction(async (tx: ExtendedTransactionClient) => {
			...
			if (!expenseToDelete) {
				console.error(
					`ExpenseService (TX): Despesa ${id} não encontrada para deleção.`,
				);
				throw new Error("Despesa não encontrada.");
			}
			if (expenseToDelete.userId !== userId) {
				console.error(
					`ExpenseService (TX): Usuário ${userId} tentando deletar despesa ${id} de outro usuário.`,
				);
				throw new Error("Acesso não permitido para deletar esta despesa.");
			}
			const amountToDelete = expenseToDelete.amount;
			console.log(
				`ExpenseService (TX): Despesa ${id} encontrada. Valor: ${amountToDelete}.`,
			);
			...
			console.log(
				`ExpenseService (TX): Recalculando saldo do usuário ${userId} após deleção.`,
			);
			await this.userService.updateUserBalance(userId, tx);
			console.log(
				`ExpenseService (TX): Saldo do usuário ${userId} recalculado.`,
	 ...
	 */
	async getExpensesByCategoryForCurrentMonth(
		userId: string,
	): Promise<{ name: string; value: number }[]> {
		console.log(
			`ExpenseService: Buscando despesas por categoria do mês atual para usuário ${userId}.`,
		);
		try {
			// Buscar o mês atual
			const hoje = new Date();
			...
			const expensesByCategory =
				await this.expenseRepository.getTotalExpensesByCategoryAndPeriod(
					userId,
					inicioMes,
					fimMes,
				);
			...
			console.log(
				`ExpenseService: Despesas por categoria do mês atual encontradas para ${userId}.`,
			);
			return formattedData;
		} catch (error) {
			console.error(
				`ExpenseService: Erro ao buscar despesas por categoria para ${userId}:`,
				error,
			);
	...
	async getRecentExpenses(
		...
		console.log(
			`ExpenseService: Buscando ${limit} despesas recentes para usuário ${userId}.`,
		);
		try {
			// Utiliza o método do repositório que já busca as recentes com limite
			const recentExpenses = await this.expenseRepository.findRecentByUserId(
				userId,
				limit,
			);
			console.log(
				`ExpenseService: ${recentExpenses.length} despesas recentes encontradas para ${userId}.`,
			);
			return recentExpenses;
		} catch (error) {
			console.error(
				`ExpenseService: Erro ao buscar despesas recentes para ${userId}:`,
				error,
			);
	...
	async getExpensesByCategoryAndPeriodReport(
		...
		startDate: Date,
		endDate: Date,
	): Promise<{ category: Category; expenses: Expense[] }> {
		// Retorna Expense básico
		console.log(
			`ExpenseService: Buscando despesas para relatório por categoria ${categoryId} e período [${startDate.toISOString()} - ${endDate.toISOString()}] para usuário ${userId}.`,
		);
		...
		try {
			...
			if (!category) {
				console.warn(
					`ExpenseService: Categoria ID ${categoryId} não encontrada ou não pertence ao usuário ${userId} para relatório.`,
				);
			...
			console.log(
				`ExpenseService: Categoria ${category.name} (ID: ${categoryId}) validada para usuário ${userId}.`,
			);
			...
			const allExpensesInPeriod =
				await this.expenseRepository.findByUserIdAndPeriod(
					userId,
					startDate,
					endDate,
				);
			...
			console.log(
				`ExpenseService: ${categoryExpenses.length} despesas encontradas para o relatório da categoria ${categoryId}.`,
			);
			...
			const expensesToReturn: Expense[] = categoryExpenses.map(
				({ category: _, ...rest }) => rest,
			);
			...
			console.error(
				`ExpenseService: Erro ao buscar despesas por categoria e período para relatório (User: ${userId}, Cat: ${categoryId}):`,
				error,
			);
			throw new Error(
				"Erro ao buscar dados para o relatório de despesas por categoria.",
			);
		}
	}
	/**
	...
	> {
		console.log(
			`ExpenseService: Buscando despesas agrupadas por categoria no período [${startDate.toISOString()} - ${endDate.toISOString()}] para usuário ${userId}.`,
		);
		try {
			...
			console.log(
				`ExpenseService: ${groupedExpenses.length} grupos de despesas por categoria encontrados para ${userId} no período.`,
			);
			// O repositório já retorna o formato { categoryId, categoryName, total } com total como number.
			return groupedExpenses;
		} catch (error) {
			console.error(
				`ExpenseService: Erro ao buscar despesas agrupadas por categoria para ${userId} no período:`,
				error,
			);
