...
class CategoryService {
  ...
  async createCategory(data: { name: string; userId: string }): Promise<Category> {
    ...
    if (!trimmedName) {
      console.error(`CategoryService: Tentativa de criar categoria sem nome para usuário ${userId}.`);
    ...
    if (existingCategory) {
        console.warn(`CategoryService: Categoria '${trimmedName}' já existe para o usuário ${userId}.`);
   ...
   */
  async updateCategory(id: string, userId: string, data: { name: string }): Promise<Category> {
    const { name } = data;
    const trimmedName = name?.trim();

    if (!trimmedName) {
      console.error(`CategoryService: Tentativa de atualizar categoria ${id} sem nome para usuário ${userId}.`);
      throw new Error('O nome da categoria não pode estar vazio.');
    }
    ...
    if (!existingCategory) {
        console.error(`CategoryService: Categoria ${id} não encontrada para atualização pelo usuário ${userId}.`);
    ...
    if (conflictingCategory) {
        console.warn(`CategoryService: Já existe outra categoria '${trimmedName}' para o usuário ${userId}.`);
  ...
  async deleteCategory(id: string, userId: string): Promise<Category> {
    ...
    if (!category) {
      console.error(`CategoryService: Categoria ${id} não encontrada para deleção pelo usuário ${userId}.`);
      throw new Error('Categoria não encontrada ou não pertence ao usuário.');
    }
    ...
    if (expenseCount > 0) {
      console.warn(`CategoryService: Categoria ${id} ('${category.name}') está em uso por ${expenseCount} despesa(s) do usuário ${userId}. Deleção bloqueada.`);
      throw new Error(`Não é possível deletar a categoria '${category.name}' pois ela está associada a ${expenseCount} despesa(s).`);
    }
