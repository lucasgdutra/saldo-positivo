...
export class ExpenseRepository {
   ...
   */
  async findById(id: string): Promise<Prisma.ExpenseGetPayload<{ include: { category: true } }> | null> {
   ...
   */
  async findByUserId(userId: string): Promise<Prisma.ExpenseGetPayload<{ include: { category: true } }>[]> {
  ...
  async getTotalExpensesByCategoryAndPeriod(userId: string, startDate: Date, endDate: Date): Promise<{ categoryId: string; categoryName: string | null; total: number }[]> {
    ...
    return result.map((item: GroupByResultItem) => {
      ...
      return {
        ...
        categoryName: item.categoryId ? categoryMap.get(item.categoryId) ?? 'Desconhecida' : 'Sem Categoria',
        total: totalAmount,
      };
    });
  }
