import { PrismaClient, User, Balance, Prisma } from '@prisma/client';
...
export class UserRepository {
   ...
   */
  async findById(id: string): Promise<(User & { balance: Balance | null }) | null> {
    return this.prisma.user.findUnique({
      where: { id },
   ...
   */
  async upsertBalance(userId: string, data: { totalAmount: number; totalRevenues: number; totalExpenses: number; referenceMonth: Date }): Promise<Balance> {
    const balanceData = {
      userId,
   ...
   */
  async updateBalancePartial(userId: string, data: Partial<{ totalAmount: number; totalRevenues: number; totalExpenses: number; referenceMonth: Date }>): Promise<Balance> {
    ...
    if (Object.keys(updateData).length === 0) {
      ...
      if (!currentBalance) {
        throw new Error(`Saldo não encontrado para o usuário com ID: ${userId}`);
      }
