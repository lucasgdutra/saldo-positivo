...
async function main() {
	...
	for (let i = 0; i < 3; i++) {
		const amount =
			i === 0 ? randomDecimal(4500, 5500) : randomDecimal(200, 800);
	...
	for (let i = 0; i < 10; i++) {
		...
		const descriptions =
			expenseDescriptions[category.name as keyof typeof expenseDescriptions];
		const description =
	...
	console.log(
		`👤 Test user created: ${SEED_USER.email} / ${SEED_USER.password}`,
	);
}
