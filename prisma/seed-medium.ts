...
async function main() {
	...
	while (currentDate <= endDate) {
		...
		for (let i = 0; i < monthlyRevenuesCount; i++) {
			...
			const baseAmount =
				i === 0 ? randomBetween(4500, 5500) : randomBetween(300, 800);
		...
		console.log(
			`   💰 ${monthRevenues.length} revenues, 💸 ${monthExpenses.length} expenses`,
		);
	...
	console.log(
		`📊 Generated ${revenueCount} revenues and ${expenseCount} expenses across 20 months`,
	);
